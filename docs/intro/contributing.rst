...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Please start a discussion on the `core repo issue tracker <https://github.com/IdentityServer/IdNet6/issues>`_.

Bugs and feature requests?
^^^^^^^^^^^^^^^^^^^^^^^^^^
Please log a new issue in the appropriate GitHub repo:

* `Core <https://github.com/IdentityServer/IdNet6>`_
* `AccessTokenValidation <https://github.com/IdentityServer/IdNet6.AccessTokenValidation>`_
...
As of October 2017, the IdNet6.* nuget namespace is reserved for our packages. Please use the following naming conventions:

``YourProjectName.IdNet6``

or

``IdNet6.Contrib.YourProjectName``
